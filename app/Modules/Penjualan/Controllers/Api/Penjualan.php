...
class Penjualan extends BaseControllerApi
{
    ...
    public function create2()
    {
        if ($this->request->getJSON()) {
            ...
            $metodeBayar = $this->request->getPost('metode_bayar');
            $catatan = $this->request->getPost('catatan');
            $idBankAkun = $this->request->getPost('id_bank_akun');
        }
    ...
    public function update($id = NULL)
    {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($bayar != 0) {
                ...
                $idToko = $penjualan['id_toko'];
                // Ambil Data Toko
                $toko = $this->toko->where('id_toko', $idToko)->first();
                // Ambil kode jual toko
                $kdJual = $toko['kode_jual'];
                $kdJualTahun = $toko['kode_jual_tahun'];
    ...
    public function cetakUSB()
    {
        ...
        $toko = $this->toko->find($penjualan['id_toko']);
        ...
        try {
            ...
            foreach ($item as $item) {
                ...
                $printer->text(str_pad("$item->qty ($item->satuan) x @ {$this->Ribuan($item->harga_jual_grosir == 0 ?$item->harga_jual :$item->harga_jual_grosir)}", 20));
