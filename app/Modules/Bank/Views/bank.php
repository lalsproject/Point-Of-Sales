...
<template>
    ...
    <v-card>
        <v-alert dense text type="primary" icon="mdi-bank-check" class="mb-0">Bank Utama
        ...
        <v-card-title>
            ...
            <v-spacer></v-spacer>
            <v-select multiple v-model="idBankAkun" label="Filter Bank" :items="dataBankAkun" item-text="nama_bank" item-value="id_bank_akun" prepend-icon="mdi-filter" @change="handleSubmit" single-line class="mr-3" style="max-width: 250px;"></v-select>

            <v-select v-model="search" label="Filter Jenis" :items="dataJenis" item-text="text" item-value="value" @change="handleSubmit" single-line clearable class="mr-3" style="max-width: 250px;"></v-select>
        ...
        <v-data-table :headers="dataTable" :items="data" :options.sync="options" :server-items-length="totalData" :items-per-page="10" :loading="loading" :search="search" loading-text="<?= lang('App.loadingWait'); ?>">
            ...
            <template v-slot:item="{ item }">
                <tr>
                    ...
                    <td>
                        <v-btn icon color="primary" class="mr-3" @click="setItem(item)" v-show="item.jenis == 'Pemasukan'" title="Mutasi" alt="Mutasi" :disabled="item.keterangan == 'Transfered'">
                            <v-icon>mdi-swap-horizontal-bold</v-icon>
                        </v-btn>

                        <v-btn icon color="error" @click="deleteItem(item)" title="Delete" alt="Delete" :disabled="item.id_penjualan != null || item.keterangan == 'Transfered'">
    ...
    <v-row justify="center">
        <v-dialog v-model="modalEdit" persistent max-width="600px">
            <v-card>
                ...
                <v-card-text class="py-5">
                    <v-form v-model="valid" ref="form">
                        <v-select v-model="idToko" label="Outlet" :items="dataToko" :item-text="dataToko =>`${dataToko.id_toko}. ${dataToko.nama_toko}, ${dataToko.alamat_toko}`" item-value="id_toko" :error-messages="id_tokoError" :loading="loading3" filled disabled></v-select>
