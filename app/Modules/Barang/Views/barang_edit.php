...
<template>
    <v-card>
        <v-toolbar elevation="1">
            ...
            <v-spacer></v-spacer>
            <v-btn @click="getBarang" large icon class="mr-3"><v-icon color="primary">mdi-refresh</v-icon></v-btn>
            ...
            <v-form ref="form" v-model="valid">
                <v-row class="mt-n2">
                    <v-col class="mb-n10" cols="12" md="3">
                        <p class="mb-1 text-subtitle-1 font-weight-bold">Outlet</p>
                        <p class="text-caption">Outlet Toko/Warung</p>
                    </v-col>
                    <v-col cols="12" md="9">
                        <v-select v-model="idToko" label="<?= lang('App.select'); ?> Outlet" :items="dataToko" :item-text="dataToko =>`${dataToko.id_toko}. ${dataToko.nama_toko}, ${dataToko.alamat_toko}`" item-value="id_toko" :error-messages="id_tokoError" :loading="loading3" @change="confirmOutlet" filled></v-select>
                    </v-col>
                </v-row>
                ...
                <v-row class="mt-n5">
                    ...
                    <v-col cols="12" md="9">
                        ...
                        <div v-else>
                            <v-file-input v-model="image" show-size label="Image Upload" id="file" class="mb-2" accept=".jpg, .jpeg, .png" prepend-icon="mdi-camera" @change="onFileChange" @click:clear="onFileClear" :loading="loading2" v-if="imagePreview == null"></v-file-input>
                            <div v-else>
                ...
                <v-row class="mt-n4">
                    ...
                    <v-col cols="12" md="9">
                        <v-row>
                            <v-col cols="12" sm="6" class="mb-n3">
                ...
                <v-row class="">
                    <v-col class="mb-n5" cols="12" md="3">
                        <p class="mb-1 text-subtitle-1 font-weight-bold"><?= lang('App.category') ?> / <?= lang('App.unit') ?></p>
                        <p class="text-caption"></p>
                    </v-col>
                    <v-col cols="12" md="9">
                        <v-row>
                            <v-col cols="12" sm="5" class="mb-n3">
                            ...
                            <v-col cols="12" sm="4" class="mb-n3">
                                <v-select v-model="idSatuan" label="<?= lang('App.select'); ?> <?= lang('App.unit') ?>" :items="dataSatuan" item-text="nama_satuan" item-value="id_satuan" :error-messages="satuan_barangError" :loading="loading2" outlined dense @change="getSatuanById" append-outer-icon="mdi-plus-thick" @click:append-outer="addSatuan"></v-select>
                            </v-col>
                            <v-col cols="12" sm="3">
                ...
                <v-row class="mt-n4">
                    <v-col class="mb-n5" cols="12" md="3">
                        ...
                        <p class="text-caption">Pastikan deskripsi barang memuat spesifikasi, ukuran, bahan, masa berlaku, dan lainnya.</p>
                    </v-col>
                    <v-col cols="12" md="9">
                        <v-textarea v-model="deskripsi" counter maxlength="3000" outlined full-width rows="3" :error-messages="deskripsiError"></v-textarea>
                    </v-col>
                </v-row>

                <v-row class="mt-n4">
                    <v-col class="mb-n5" cols="12" md="3">
                        <p class="mb-2 text-subtitle-1 font-weight-bold"><?= lang('App.price') . ' ' . lang('App.unit'); ?></p>
                        <p class="text-caption"></p>
                    </v-col>
                    <v-col cols="12" md="9">
                        <v-row class="mb-n7">
                            <v-col cols="12" sm="4">
                                <v-text-field label="<?= lang('App.priceBuy') ?> *" v-model="hargaBeli" type="number" @input="countHargajual" :error-messages="harga_beliError" prefix="Rp" outlined></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="3">
                                <v-text-field label="x Margin %" v-model="marginBarang" type="number" @input="countHargajual" :loading="loading5" :error-messages="marginError" outlined></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="5">
                                <v-text-field label="<?= lang('App.priceSell') ?> *" v-model="hargaJual" type="number" :error-messages="harga_jualError" prefix="Rp" outlined></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row>
                            <v-col cols="12" sm="6">
                                <v-checkbox v-model="checkDiskon" label="Aktifkan Diskon <?= lang('App.priceSell') ?>" class="mt-n4"></v-checkbox>
                            </v-col>
                            <v-col cols="12" sm="6">
                                <v-text-field v-model="diskon" label="<?= lang('App.discount') ?> (Rp)" type="number" :error-messages="diskonError" prefix="Rp" :suffix=" diskonPersen.toFixed() + '%'" @focus="$event.target.select()" outlined v-show="checkDiskon == true"></v-text-field>
                            </v-col>
                        </v-row>
                    </v-col>
                </v-row>
                ...
                <v-row class="mt-n4">
                    <v-col class="mb-n5" cols="12" md="3">
                        <p class="mb-2 text-subtitle-1 font-weight-bold"><?= lang('App.price') . ' ' . lang('App.wholesale'); ?></p>
                        <p class="text-caption"></p>
                    </v-col>
                    <v-col cols="12" md="9">
                        <v-checkbox v-model="checkGrosir" label="Aktifkan <?= lang('App.wholesale'); ?>" class="mt-1"></v-checkbox>
                        <v-row class="mb-n7" v-show="checkGrosir == true">
                            <v-col cols="12" sm="5">
                                <v-text-field label="<?= lang('App.minQty') ?> *" v-model="jumlahMinGrosir" type="number" :error-messages="jumlah_min_grosirError" min="0" prefix="&ge;" dense outlined></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="5">
                                <v-text-field label="<?= lang('App.wholesalePrice') ?> *" v-model="hargaJualGrosir" type="number" :error-messages="harga_jual_grosirError" prefix="Rp" dense outlined></v-text-field>
                            </v-col>
                        </v-row>
                    </v-col>
                </v-row>
                ...
                <v-row class="mt-0">
                    <v-col class="mb-n5" cols="12" md="3">
                        <p class="mb-1 text-subtitle-1 font-weight-bold">Vendor/Supplier</p>
                        <p class="text-caption"></p>
                    </v-col>
                    <v-col cols="12" md="9">
...
<script>
    ...
    dataVue = {
        ...
        checkGrosir: false,
        jumlahMinGrosir: 0,
        jumlah_min_grosirError: "",
        hargaJualGrosir: 0,
        harga_jual_grosirError: ""
    }
    ...
    watchVue = {
        ...
        hargaJualGrosir: function() {
            if (Number(this.hargaJualGrosir) >= Number(this.hargaJual)) {
                this.harga_jual_grosirError = "Harga grosir harus lebih murah dari harga jual satuan";
            } else {
                this.harga_jual_grosirError = "";
            }
        }
    }
    ...
    methodsVue = {
        ...methodsVue,
        // Hitung Harga jual x margin %
        countHargajual: function() {
            let persen = (this.marginBarang / 100) * Number(this.hargaBeli);
            this.hargaJual = Number(this.hargaBeli) + persen;
        },
            ...
            axios.get(`<?= base_url(); ?>api/barang/${this.idBarang}`, options)
                .then(res => {
                    ...
                    if (data.status == true) {
                        ...
                        this.idToko = this.dataBarang.id_toko;
                        this.marginBarang = this.dataBarang.margin;
                        this.jumlahMinGrosir = this.dataBarang.jumlah_min_grosir;
                        this.hargaJualGrosir = this.dataBarang.harga_jual_grosir;
                        if (Number(this.jumlahMinGrosir) > 0 || Number(this.hargaJualGrosir) > 0) {
                            this.checkGrosir = true;
                        }
        ...
        updateProduct: function() {
            ...
            if (this.checkGrosir == false) {
                var jumlahMinGrosir = 0;
                var hargaJualGrosir = 0;
            } else {
                var jumlahMinGrosir = this.jumlahMinGrosir;
                var hargaJualGrosir = this.hargaJualGrosir;
            }
            axios.put(`<?= base_url(); ?>api/barang/update/${this.idBarang}`, {
                    ...
                    id_toko: this.idToko,
                    margin: this.marginBarang,
                    jumlah_min_grosir: jumlahMinGrosir,
        ...
        confirmOutlet: function() {
            if (confirm('<?= lang("App.confirmOutletEditBrg"); ?>')) {
                this.getTokoById();
            } else {
                this.getBarang();
            }
        ...
        getTokoById: function() {
            ...
            axios.get(`<?= base_url(); ?>api/toko/${this.idToko}`, options)
                .then(res => {
                    ...
                    if (data.status == true) {
                        this.toko = data.data;
                        this.marginBarang = this.toko.margin_barang;
                        this.marginError = "<?= lang('App.marginChanged'); ?>";
                        setTimeout(() => this.marginError = "", 5000);
                        this.snackbar = true;
                        this.snackbarMessage = "<?= lang('App.marginChanged'); ?>";
                        this.countHargajual();
