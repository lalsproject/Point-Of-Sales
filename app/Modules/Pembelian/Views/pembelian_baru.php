    ...
    <v-card>
        ...
        <v-card-text>
            <v-form ref="form" v-model="valid">
                <p class="mb-2 text-subtitle-1">Outlet</p>
                <v-select v-model="idToko" label="<?= lang('App.select'); ?> Outlet" :items="dataToko" :item-text="dataToko =>`${dataToko.id_toko}. ${dataToko.nama_toko}, ${dataToko.alamat_toko}`" item-value="id_toko" :error-messages="id_tokoError" outlined></v-select>

                <p class="mb-2 text-subtitle-1">Vendor/Supplier</p>
                <v-autocomplete v-model="idKontak" label="<?= lang('App.select'); ?> Kontak" :items="dataVendor" :item-text="dataVendor =>`${dataVendor.nama} - ${dataVendor.perusahaan} - ${dataVendor.telepon}`" item-value="id_kontak" prepend-inner-icon="mdi-account" :error-messages="id_kontakError" :loading="loading3" outlined :disabled="idKontak == '0'">
                    <template v-slot:prepend-item>
                        <v-subheader class="mt-n3 mb-n3">{{ dataVendor.length }} Vendor/Supplier found</v-subheader>
                    </template>
                </v-autocomplete>
                <v-checkbox v-model="noVendor" @change="clicknoVendor" label="No Vendor/Supplier" class="mt-n2"></v-checkbox>
            ...
            <v-autocomplete v-model="idBarang" label="<?= lang('App.select'); ?> <?= lang('App.items'); ?>/Barcode/SKU" :items="barang" :item-text="barang =>`${barang.nama_barang} - Rp${barang.harga_beli} - Stok: ${barang.stok} - Barcode: ${barang.barcode} - SKU: ${barang.sku}`" item-value="id_barang" :disabled="idKontak == ''" :loading="loading3" outlined>
