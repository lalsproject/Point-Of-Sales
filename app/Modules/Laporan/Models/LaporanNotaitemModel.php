...
class LaporanNotaitemModel extends Model
{
    ...
    {
        $this->select("t.nama_toko, p.faktur, p.subtotal, p.diskon, p.diskon_persen, p.total, p.pajak, p.pembulatan, {$this->table}.qty, {$this->table}.jumlah, {$this->table}.satuan, b.id_barang, b.kode_barang, b.nama_barang, {$this->table}.created_at");
        $this->join("barang b", "b.id_barang = {$this->table}.id_barang");
        $this->join("penjualan p", "p.id_penjualan = {$this->table}.id_penjualan");
        $this->join("toko t", "t.id_toko = p.id_toko");
