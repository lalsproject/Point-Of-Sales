...
<body>
        ...
        <v-app>
            ...
            <v-app-bar app color="white" light elevation="2">
                ...
                <?php if (session()->get('role') == 1 || session()->get('role') == 2 || session()->get('role') == 3) : ?>
                    <v-btn color="grey darken-3" class="pa-2 me-2" outlined elevation="0" link href="<?= base_url('sales'); ?>">
                        <v-icon>mdi-cash-register</v-icon> <?= lang('App.sell'); ?>
                    </v-btn>
                <?php endif; ?>
                <?php if (session()->get('role') == 1 || session()->get('role') == 3) : ?>
                    <v-btn color="grey darken-3" class="pa-2" outlined elevation="0" link href="<?= base_url('pembelian'); ?>">
                ...
                <?php if (!empty(session()->get('username'))) : ?>
                    <v-menu offset-y>
                        ...
                        <v-list>
                            ...
                            <v-list-item @click="openChangeStore">
                                <v-list-item-icon>
                                    <v-icon>mdi-store</v-icon>
                                </v-list-item-icon>
                                <v-list-item-content>
                                    <v-list-item-title><?= lang('App.storeLocation'); ?></v-list-item-title>
                                    <v-list-item-subtitle><?= lang('App.store'); ?>: {{toko.nama_toko ?? "-"}}</v-list-item-subtitle>
                                </v-list-item-content>
                                <v-list-item-action>
                                    <v-icon color="warning" v-if="toko == ''">mdi-alert-circle</v-icon>
            ...
            <v-dialog v-model="modalStore" persistent scrollable width="700">
                <v-card>
                    <v-card-title class="text-h5 grey lighten-2 mb-1">
                        <?= lang('App.storeLocation'); ?>&nbsp;<v-icon>mdi-store-marker</v-icon>
                    </v-card-title>
                    <v-card-subtitle class="grey lighten-2">
                        <?= lang('App.select'); ?> <?= lang('App.storeLocation'); ?> untuk halaman Sales &amp; <?= lang('App.statistic'); ?>. (Please Allow Cookies in your Browser)
                    </v-card-subtitle>

                    <v-card-text class="py-5">
                        <v-row v-if="loading1 == true">
                            <v-col v-for="n in 2" :key="n" cols="12" sm="6">
                                <v-skeleton-loader class="mx-auto" max-width="300" max-height="160" type="card"></v-skeleton-loader>
                            </v-col>
                        </v-row>
                        <v-row v-else>
                            <v-col v-for="item in dataOutlet" :key="item.id_toko">
                                <v-card link @click="setToko(item)">
                                    <v-card-title>
                                        {{item.nama_toko}}
                                        <v-spacer></v-spacer>
                                        <v-chip color="green" class="white--text" small v-show="item.id_toko == localStorage.getItem('id_toko')">
                                            <v-icon small>mdi-check</v-icon> &nbsp;Selected
                                        </v-chip>
                                    </v-card-title>
                                    <v-card-text>
                                        <div>
                                            NIB: {{item.NIB ?? "-"}}<br />
                                            <span class="d-inline-block  text-truncate" style="max-width: 300px;"><v-icon small>mdi-map-marker</v-icon> {{item.alamat_toko}}</span>
                                        </div>
                                    </v-card-text>
                                </v-card>
                            </v-col>
                    ...
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <!-- <v-btn color="primary" text @click="modalStore = false">
                            Close
                        </v-btn> -->
                    </v-card-actions>
                </v-card>
    ...
    <script>
        ...
        var mountedVue = function() {
            ...
            const localeToko = localStorage.getItem("id_toko");
            const cookieToko = "<?= get_cookie('id_toko'); ?>";
            if (localeToko && cookieToko) {
                this.getOutletById();
            } else if (localeToko == null) {
                this.openChangeStore();
            } else if (cookieToko == "") {
                this.openChangeStore();
            }
        }
        ...
        var methodsVue = {
            ...
            getOutlet: function() {
                ...
                axios.get('<?= base_url(); ?>api/toko', options)
                    .then(res => {
                        ...
                        var data = res.data;
                        if (data.status == true) {
                            this.dataOutlet = data.data;
                            //console.log(this.dataOutlet);
                        } else {
                            //this.snackbar = true;
            ...
            // Set Toko to localStorage
            setToko: function(item) {
                axios.post('<?= base_url('api/toko/settoko') ?>', {
                    ...
                    .then(res => {
                        ...
                        var data = res.data;
                        if (data.status == true) {
                            localStorage.setItem("id_toko", item.id_toko);
            ...
            //Get Toko By ID
            getOutletById: function() {
                axios.get(`<?= base_url(); ?>api/toko/${localStorage.getItem('id_toko')}`, options)
                    .then(res => {
                        // handle success
                        var data = res.data;
                        if (data.status == true) {
